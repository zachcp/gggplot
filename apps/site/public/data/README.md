...
The site keeps CSV assets separate from the rendering pipeline. The async
loader in `src/docs/data/real.ts` converts each asset directly to a typed
column store; a static asset is suitable for `RawData`, while changing streams
must use the explicit `GPUStreamingSourceAdapter` rather than emulate ranges
through a static source.
