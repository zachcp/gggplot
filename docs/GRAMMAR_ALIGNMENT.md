...
- [Gribouille](https://github.com/mcanouil/gribouille), a Typst-native
  Grammar of Graphics library.
- [ggsql](https://ggsql.org/), a SQL extension for declarative visualization.

The purpose is not to copy either project wholesale. gggplot's center of
gravity is different: it is a TypeScript DSL that lowers a serializable
`GGSpec` into a backend-independent `RenderTree`, then renders live through
UseGPU or emits UseGPU Live source. Gribouille is document-native; ggsql is
query-native; gggplot is render-backend-native.
...
1. Keep the RenderTree-first architecture.
   gggplot should not become a Typst clone or SQL dialect. The durable design
   choice is that both runtime rendering and emitted source consume the same
   backend-independent tree.

2. Make grammar concepts explicit before adding surface area.
   Peer dialects expose labels, projection, annotations, and literal settings
   as first-class grammar concepts. gggplot should prefer clear IR fields over
   ad hoc compiler fallbacks.
...
4. Keep aesthetics independent.
   `color`, `fill`, and future `stroke` should not share one compiler scale
   shortcut. Each aesthetic should train, guide, and render independently unless
   an explicit API says otherwise.

5. Do not export grammar promises that compile to nothing.
   A public DSL helper should either lower to a RenderTree node or be clearly
   marked unsupported. The current `geomPolygon()` gap violates this.
...
| Coordinate projection | yes | ggsql `PROJECT` names position aesthetics and coordinate system. | `gggplot-1be.3` replaced the cartesian-only "flip" coord kind with a `Coord.project: [PositionAxis, PositionAxis]` field shared by every coord kind. |
| Polar | partial | ggsql treats polar as a coordinate system; gggplot has polar with polygon munching. | `coord_polar(theta: "y")` now reassigns the angle via the same projection field `coordFlip()` uses; munching still needs to extend to paths/lines. |
...
6. `gggplot-1be.4`: add a first annotation/literal layer API — done via
   ...
   trained domain.
7. `gggplot-1be.3`: generalize coordinate projection once the user-facing
   grammar vocabulary is clearer — done: `Coord.project` replaces the
   cartesian-only `"flip"` coord kind, so `coordFlip()` and
   `coordPolar({ theta: "y" })` both reduce to the same `["y","x"]`
   projection swap, applied via the identical `axes` swizzle trait that both
   `Cartesian` and `Polar` already accept from `@use-gpu/plot`.

## Open Design Questions

- Should gggplot use American `color` only, or provide `colour` aliases to
  match ggplot2/Gribouille? The current TypeScript API uses `color`.
